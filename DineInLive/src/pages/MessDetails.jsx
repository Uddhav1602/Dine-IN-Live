...
const MessDetails = () => {
  ...
  const checkIfFavorite = (messId) => {
    ...
    setIsFavorite(favorites.some((fav) => fav.id === messId));
  };
  ...
  if (!mess) return (
    <div className="h-screen flex flex-col items-center justify-center">
      <p className="text-2xl font-bold text-[#5C2E00] mb-4">Mess not found</p>
      <button onClick={() => navigate("/search")} className="bg-[#D2691E] text-white px-6 py-2 rounded-lg hover:bg-[#8B4513] transition">
        Back to Search
      </button>
    </div>
  );
      ...
      <div className="relative h-48 sm:h-64 bg-gradient-to-br from-[#8B5A2B] to-[#5C2E00]">
        ...
        <div className="absolute bottom-0 left-0 w-full p-4 sm:p-6 bg-gradient-to-t from-black/80 to-transparent text-white">
          <div className="container mx-auto flex flex-col sm:flex-row justify-between items-start sm:items-end gap-3">
            <div>
              <h1 className="text-2xl sm:text-4xl font-bold mb-1">{mess.name}</h1>
              <p className="text-base sm:text-lg opacity-90">📍 {mess.location}</p>
              {mess.fullAddress && (
                <a
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-full font-bold transition shadow-lg text-sm sm:text-base ${
                isFavorite ? "bg-white text-red-500 hover:bg-gray-100" : "bg-red-500 text-white hover:bg-red-600"
              }`}
            >
      ...
      <div className="flex-1 container mx-auto px-4 py-6 max-w-5xl">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
            <h2 className="text-2xl font-bold text-[#5C2E00] mb-4 border-b-2 border-[#D2691E] pb-2">Menu</h2>
                ...
                {mess.menuItems.map((item, index) => (
                  ...
                  >
                    <div className="flex-1 mr-4">
                      <h4 className="font-bold text-base sm:text-lg text-[#3B1E00]">{item.name}</h4>
                      {item.contents && item.contents !== "Single Item" && (
                        <p className="text-xs sm:text-sm text-gray-500 mt-0.5">{item.contents}</p>
                      )}
                    </div>
                    <div className="text-right shrink-0">
                      <p className="font-bold text-[#D2691E] text-base sm:text-lg">₹{item.price}</p>
                      <button
            ...
            <div className="lg:w-80">
              <div className="bg-white p-5 rounded-xl shadow-xl border-t-4 border-[#D2691E] sticky top-4">
                ...
                <div className="max-h-60 overflow-y-auto space-y-2 mb-4">
                  {cart.map((item) => (
                    <div key={item.name} className="flex justify-between items-center text-sm border-b pb-1.5">
                      <div>
                        <span className="font-medium">{item.name}</span>
                        <span className="text-gray-400 ml-1 text-xs">x{item.quantity}</span>
                      </div>
