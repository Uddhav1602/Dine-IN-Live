...
const Login = () => {
  ...
  const handleSignup = async (e) => {
    ...
    try {
      ...
      setUsername(""); setEmail(""); setPhone(""); setAddress(""); setPassword("");
      setErrorMsg("");
      ...
      <div className="relative z-10 overflow-hidden w-full max-w-[768px] min-h-[550px] bg-white rounded-[30px] shadow-2xl flex flex-col md:block">
        ...
        <div className="flex md:hidden w-full z-50 bg-white">
          <button
            ...
            className={`flex-1 py-4 text-base font-bold transition-colors duration-300 ${!isSignUp ? 'bg-[#D2691E] text-white' : 'bg-gray-100 text-gray-500'}`}
          >
            Sign In
          </button>
          <button
        ...
        <div
          className={`
            ...
            ${isSignUp ? "md:translate-x-full md:opacity-100 md:z-50 block" : "md:opacity-0 md:z-0 hidden md:block"}
          `}
        >
          <form onSubmit={handleSignup} className="flex flex-col items-center justify-center h-full text-center">
            <h1 className="text-2xl sm:text-3xl font-bold mb-4 text-[#8B4513]">Create Account</h1>
            ...
            <input className="input text-sm md:text-base" type="email" placeholder="Email" value={email} onChange={e => setEmail(e.target.value)} />
            <input className="input text-sm md:text-base" type="tel" placeholder="Phone" value={phone} onChange={e => setPhone(e.target.value)} />
            <input className="input text-sm md:text-base" placeholder="Address" value={address} onChange={e => setAddress(e.target.value)} />
            <input type="password" className="input text-sm md:text-base" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} />

            <button type="submit" disabled={signupLoading} className="btn mt-4 disabled:opacity-60">{signupLoading ? "Signing Up..." : "Sign Up"}</button>
          </form>
        </div>
        ...
        <div
          className={`
            md:absolute md:top-0 md:h-full md:w-1/2 md:left-0 md:transition-all md:duration-700 md:ease-in-out
            flex-1 w-full p-6 sm:p-10
            ${isSignUp ? "md:translate-x-full md:opacity-0 hidden md:block" : "md:opacity-100 md:z-20 block"}
          `}
        >
          <form onSubmit={handleLogin} className="flex flex-col items-center justify-center h-full text-center">
            <h1 className="text-2xl sm:text-3xl font-bold mb-4 text-[#8B4513]">Sign In</h1>
            ...
            <input className="input text-sm md:text-base" placeholder="Username" value={loginUsername} onChange={e => setLoginUsername(e.target.value)} />
            <input type="password" className="input text-sm md:text-base" placeholder="Password" value={loginPassword} onChange={e => setLoginPassword(e.target.value)} />
            
            <div className="my-2 text-sm text-gray-500 hover:text-[#D2691E] cursor-pointer">Forgot your password?</div>
            
            <button type="submit" disabled={loginLoading} className="btn mt-4 disabled:opacity-60">{loginLoading ? "Signing In..." : "Sign In"}</button>
          </form>
        </div>
        ...
        >
          ...
          >
            ...
            <div className="absolute top-0 w-1/2 h-full flex flex-col items-center justify-center px-10 text-center transform translate-x-0 transition-transform duration-700">
              ...
              <button type="button" onClick={() => { setIsSignUp(false); setErrorMsg(""); setSuccessMsg(""); }} className="overlay-btn">
                Sign In
              </button>
            </div>
            ...
            <div className="absolute top-0 right-0 w-1/2 h-full flex flex-col items-center justify-center px-10 text-center transform translate-x-0 transition-transform duration-700">
              ...
              <button type="button" onClick={() => { setIsSignUp(true); setErrorMsg(""); setSuccessMsg(""); }} className="overlay-btn">
                Sign Up
              </button>
            </div>
          </div>
